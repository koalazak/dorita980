...
var dorita980 = function localV2 (user, password, host, emitIntervalTime) {
  ...
  var customOptions = {};
  if (emitIntervalTime && typeof emitIntervalTime === 'object') {
    customOptions = emitIntervalTime;
    emitIntervalTime = customOptions.emitIntervalTime;
  }
  ...
  if (typeof customOptions.secureOptions !== 'undefined') {
    options.secureOptions = customOptions.secureOptions;
  } else if (process.env.ROBOT_TLS_LEGACY !== '0' && constants && constants.SSL_OP_LEGACY_SERVER_CONNECT) {
  ...
  client.on('error', function (e) {
    if (customOptions.onError) return customOptions.onError(e);
    // Ignore by default to avoid crashing the host process.
  });
